...
struct MathListTests {
    ...
    @Test func append() {
        ...
        let a2 = MTMathAtomFactory.placeholder()
        let a3 = MTMathAtomFactory.placeholder()
        list1.addAtom(a1); list1.addAtom(a2); list1.addAtom(a3)

        let list2 = MTMathList()
        let a4 = MTMathAtomFactory.times()
        let a5 = MTMathAtomFactory.divide()
        list2.addAtom(a4); list2.addAtom(a5)
    ...
    @Test func removeAtAtIndex() {
        ...
        let a1 = MTMathAtomFactory.placeholder()
        let a2 = MTMathAtomFactory.placeholder()
        list.addAtom(a1); list.addAtom(a2)
        #expect(list.atoms.count == 2)
    ...
    @Test func removeAtomsInRange() {
        ...
        let a2 = MTMathAtomFactory.placeholder()
        let a3 = MTMathAtomFactory.placeholder()
        list.addAtom(a1); list.addAtom(a2); list.addAtom(a3)
        #expect(list.atoms.count == 3)
        list.removeAtoms(in: NSRange(location: 1, length: 2))
        #expect(list.atoms.count == 1)
        #expect(list.atoms[0] === a1)
        #expect(ObjCExceptionCatcher.catchException { list.removeAtoms(in: NSRange(location: 1, length: 3)) })
    }
    ...
    private func checkFinalizedList(_ list: MTMathList) {
        ...
        let a0 = list.atoms[0]
        #expect(a0.type == .unaryOperator); #expect(a0.nucleus == "−")
        #expect(a0.indexRange == NSRange(location: 0, length: 1))

        let a1 = list.atoms[1]
        #expect(a1.type == .number); #expect(a1.nucleus == "52")
        #expect(a1.indexRange == NSRange(location: 1, length: 2))

        let a2 = list.atoms[2]
        #expect(a2.type == .variable); #expect(a2.nucleus == "x")
        #expect(a2.indexRange == NSRange(location: 3, length: 1))

        let superScr = a2.superScript!
        #expect(superScr.atoms.count == 3)
        #expect(superScr.atoms[0].type == .number);        #expect(superScr.atoms[0].nucleus == "13")
        #expect(superScr.atoms[0].indexRange == NSRange(location: 0, length: 2))
        #expect(superScr.atoms[1].type == .binaryOperator); #expect(superScr.atoms[1].nucleus == "+")
        #expect(superScr.atoms[2].type == .variable);      #expect(superScr.atoms[2].nucleus == "y")

        let subScr = a2.subScript!
        #expect(subScr.atoms.count == 2)
        #expect(subScr.atoms[0].type == .number);         #expect(subScr.atoms[0].nucleus == "15")
        #expect(subScr.atoms[1].type == .unaryOperator);  #expect(subScr.atoms[1].nucleus == "−")

        #expect(list.atoms[3].type == .binaryOperator); #expect(list.atoms[3].nucleus == "+")
        #expect(list.atoms[4].type == .open);           #expect(list.atoms[4].nucleus == "(")
        #expect(list.atoms[5].type == .unaryOperator);  #expect(list.atoms[5].nucleus == "−")
        #expect(list.atoms[6].type == .number);         #expect(list.atoms[6].nucleus == "12.3")
        #expect(list.atoms[7].type == .unaryOperator);  #expect(list.atoms[7].nucleus == "*")
        #expect(list.atoms[8].type == .close);          #expect(list.atoms[8].nucleus == ")")
        ...
        #expect(numer.atoms[0].type == .unaryOperator); #expect(numer.atoms[0].nucleus == "−")
        #expect(numer.atoms[1].type == .number);        #expect(numer.atoms[1].nucleus == "12")

        let denom = frac.denominator
        #expect(denom.atoms.count == 1)
        #expect(denom.atoms[0].type == .number); #expect(denom.atoms[0].nucleus == "15.2")
    }
}
